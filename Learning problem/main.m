...
bdim = 100;   % dimension of the function space
order = 2;  % order of the spline basis

ntrial = 1;
np = 50*ones(ntrial,1); % vector of number of agents. For every entry np(i) an initial datum
...
for trial = 1:length(np)
    ...
    cvx_begin
        ...
        minimize( norm(C'*x - d') )
        subject to
            norm(x,Inf) + norm(diff*x,Inf) <= Mconstr;
    cvx_end
    y_0(:,trial) = x;
